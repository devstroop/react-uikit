  ...
  for (const theme of THEMES) {
    test(`theme=${theme} light — no axe violations`, async ({ page }) => {
      ...
      const darkToggle = page.locator(
        ".chrome-controls input[type='checkbox']"
      );
...
test.describe('keyboard & a11y — core primitives', () => {
  ...
  test('Dialog — Esc closes', async ({ page }) => {
    ...
    const trigger = page
      .getByRole('button', { name: /open dialog|dialog/i })
      .first();
  ...
  test('Field — error links via aria-describedby & aria-live', async ({
    ...
    const fieldError = page
      .locator(".dt-field .dt-error, [aria-live='polite']")
      .first();
    if (!(await fieldError.count())) test.skip();
    await expect(fieldError).toHaveAttribute('aria-live', 'polite');
    const input = page
      .locator("input[aria-invalid='true'], input[aria-describedby]")
      .first();
