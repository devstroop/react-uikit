...
describe("shade ramp tokens", () => {
  it.each(HUES)("defines all four --dx-color-%s-* fill steps", (hue) => {
    for (const step of STEPS) {
      expect(css).toContain(`--dx-color-${hue}-${step}:`);
    }
  });
  ...
  it("defines each fill step in all three theme blocks (light, dark, OS fallback)", () => {
    for (const hue of HUES) {
      for (const step of STEPS) {
        const matches = css.match(new RegExp(`--dx-color-${hue}-${step}:\\s*${HEX_IN_CSS};`, "g"));
  ...
  it("uses valid lowercase hex for every ramp value", () => {
    for (const hue of HUES) {
      for (const step of STEPS) {
        const values = [...css.matchAll(new RegExp(`--dx-color-${hue}-${step}:\\s*([^;]+);`, "g"))].map(
          (m) => (m[1] ?? "").trim(),
        );
    ...
    for (const hue of ["primary", "secondary", "info", "success", "warning", "danger"] as const) {
      for (const role of ["border", "outline"] as const) {
        for (const step of ["lighter", "dark"] as const) {
          const matches = css.match(new RegExp(`--dx-color-${role}-${hue}-${step}:\\s*${HEX_IN_CSS};`, "g"));
  ...
  it("defines on-* foreground aliases for every step in all three theme blocks", () => {
    for (const hue of HUES) {
      for (const step of STEPS) {
        const matches = css.match(new RegExp(`--dx-color-on-${hue}-${step}:\\s*var\\([^;]+\\);`, "g"));
  ...
  it("points every on-* alias at an existing token", () => {
    const defined = new Set(
      [...css.matchAll(/--dx-color-[a-z0-9-]+(?=:)/g)].map((m) => m[0]),
    );
    for (const m of css.matchAll(/--dx-color-on-[a-z]+-(?:lighter|light|dark|darker):\s*var\((--dx-color-[a-z-]+)\)/g)) {
      const target = m[1] ?? "";
