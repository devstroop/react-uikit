...
const byName = (name: string): FilterDescriptor => ({
  property: 'name',
  operator: 'Contains',
  value: name,
});
...
describe('applyFilters', () => {
  it('Contains is case-insensitive by default and honors caseSensitivity', () => {
    expect(applyFilters(people, byName('a')).map((p) => p.name)).toEqual([
      'jane',
      'ALICE',
    ]);
    expect(
      applyFilters(people, byName('a'), {
  ...
  it('IsNull / IsNotNull / IsEmpty / IsNotEmpty', () => {
    expect(
      applyFilters(people, { property: 'address', operator: 'IsNull' }).map(
        (p) => p.name
      )
    ).toEqual(['jane', 'ALICE']);
    expect(
      applyFilters(people, { property: 'address', operator: 'IsNotNull' })
    ).toHaveLength(2);
    expect(
      applyFilters(people, { property: 'name', operator: 'IsEmpty' })
    ).toHaveLength(0);
    expect(
      applyFilters(people, { property: 'name', operator: 'IsNotEmpty' })
    ).toHaveLength(4);
  });
...
describe('matchesFilters / composite descriptors', () => {
  it('And group requires all children', () => {
    const group: CompositeFilterDescriptor = {
      ...
      filters: [
        byName('o'),
        { property: 'age', operator: 'GreaterThan', value: 20 },
      ],
    };
  ...
  it('Or group requires any child', () => {
    const group: CompositeFilterDescriptor = {
      ...
      filters: [
        byName('z'),
        { property: 'age', operator: 'Equals', value: 22 },
      ],
    };
  ...
  it('second operator pair combines with logicalOperator', () => {
    ...
    expect(applyFilters(people, between).map((p) => p.name)).toEqual([
      'John',
      'jane',
    ]);
  });
  ...
  it('renders a valueless nullish second clause in both string forms', () => {
    ...
    expect(toFilterString(descriptor)).toBe(
      '(age.GreaterThan(20) And age == null)'
    );
    expect(toODataFilterString(descriptor)).toBe('(age gt 20 and age eq null)');
  });

  it('empty group matches everything', () => {
    expect(applyFilters(people, { operator: 'And', filters: [] })).toHaveLength(
      4
    );
  });
});
...
describe('toFilterString', () => {
  ...
  it('renders composite groups with And/Or keywords', () => {
    const group: CompositeFilterDescriptor = {
      ...
      filters: [
        byName('jo'),
        { property: 'age', operator: 'GreaterThan', value: 21 },
      ],
    };
    expect(toFilterString(group)).toBe(
      '(name.Contains("jo") And age.GreaterThan(21))'
    );
  });

  it('escapes quotes in values', () => {
    expect(
      toFilterString({ property: 'name', operator: 'Equals', value: 'O"Brien' })
    ).toBe('name.Equals("O\\"Brien")');
  });

  it('renders null/empty operators', () => {
    expect(toFilterString({ property: 'address', operator: 'IsNull' })).toBe(
      'address == null'
    );
    expect(toFilterString({ property: 'name', operator: 'IsNotEmpty' })).toBe(
      'name != ""'
    );
  });
});
...
describe('toODataFilterString', () => {
  it('renders contains in OData style with tolower for case-insensitive', () => {
    expect(toODataFilterString(byName('jo'))).toBe(
      "contains(tolower(name), tolower('jo'))"
    );
    expect(
      toODataFilterString(byName('jo'), { caseSensitivity: 'CaseSensitive' })
    ).toBe("contains(name, 'jo')");
  });
  ...
  it('renders composite groups with and/or', () => {
    ...
    expect(toODataFilterString(group)).toBe(
      "(contains(tolower(name), tolower('jo')) and age gt 21)"
    );
  });

  it('renders null/empty operators', () => {
    expect(
      toODataFilterString({ property: 'address', operator: 'IsNull' })
    ).toBe('address eq null');
    expect(toODataFilterString({ property: 'name', operator: 'IsEmpty' })).toBe(
      "name eq ''"
    );
  });
  ...
  it('escapes single quotes', () => {
    expect(
      toODataFilterString(
        { property: 'name', operator: 'Equals', value: "O'Brien" },
        { caseSensitivity: 'CaseSensitive' }
      )
...
describe('sortItems', () => {
  it('sorts ascending by default descriptor order', () => {
    expect(
      sortItems(people, [{ property: 'age', sortOrder: 'Ascending' }]).map(
        (p) => p.name
      )
    ).toEqual(['ALICE', 'jane', 'John', 'Bob']);
  });

  it('multi-descriptor sorts tie-break in order', () => {
    const sorted = sortItems(people, [
