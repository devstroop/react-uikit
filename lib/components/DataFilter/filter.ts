...
export const DEFAULT_OPERATOR_BY_TYPE: Record<
  FilterPropertyType,
  FilterOperator
> = {
  string: 'Contains',
...
function matchesDescriptor(
  ...
  const secondary = testValue(
    actual,
    descriptor.secondValue,
    descriptor.secondOperator as FilterOperator,
    caseSensitivity
  );
  return (descriptor.logicalOperator ?? 'And') === 'And'
    ? primary && secondary
    : primary || secondary;
}
...
function testValue(
  ...
  switch (operator) {
    case 'Equals':
      return (
        a === e ||
        (Array.isArray(a) && a.some((entry) => normalize(entry) === e))
      );
    case 'NotEquals':
      return (
        a !== e &&
...
export function sortItems<T>(
  items: readonly T[],
  sortDescriptors: readonly SortDescriptor[]
): T[] {
  ...
  return [...items].sort((a, b) => {
    for (const descriptor of sortDescriptors) {
      ...
      const result = compare(
        getByPath(a, descriptor.property),
        getByPath(b, descriptor.property)
      );
