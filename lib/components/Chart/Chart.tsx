...
export interface ChartSeries {
  type:
    'line' | 'area' | 'bar' | 'column' | 'scatter' | 'bubble' | 'pie' | 'donut';
        ...
        {(() => {
          ...
          for (const ser of series) {
            ...
            for (const d of ser.data) {
              ...
              if (!stackSums.has(ser.stack))
                stackSums.set(ser.stack, new Map());
          ...
          const pieGroups = series.filter(
            (s) => s.type === 'pie' || s.type === 'donut'
          );
          const pieTotals = new Map<object, number>();
          for (const ser of pieGroups) {
            const total = ser.data.reduce(
              (sum, d) => sum + (Number(d[ser.valueProperty]) || 0),
              0
            );
          ...
          return series.map((ser, sIdx) => {
            ...
            if (ser.type === 'pie' || ser.type === 'donut') {
              ...
              const innerR =
                ser.type === 'donut' ? (ser.innerRadius ?? outerR * 0.5) : 0;
              const total =
                pieTotals.get(ser) ?? pts.reduce((s, p) => s + p.val, 0);
              let angle = -90;
              return (
                <g
                  key={sIdx}
                ...
                >
                  ...
                  {pts.map((p, i) => {
                    ...
                    const x = Number.isNaN(xVal)
                      ? xFor(ci)
                      : pad.l +
                        ((xVal - scale.min) / (scale.max - scale.min || 1)) *
                          plotW;
                    const y = yFor(p.val);
                    const r =
            ...
            if (ser.type === 'line' || ser.type === 'area') {
              ...
              return (
                ...
                >
                  ...
                  {ser.stack && (
                    <path d={baseD} fill="none" stroke="transparent" />
                  )}
                  {pts.map((p, i) => {
                    const ci = cIdxMap.get(p.cat) ?? 0;
                    const base = baseFor(p.cat);
                ...
                {pts.map((p, i) => {
                  ...
                  if (ser.stack) {
                    for (let j = 0; j < sIdx; j++) {
                      ...
                      if (found)
                        stackOffset += Number(found[prev.valueProperty]) || 0;
                    }
                  }
                  const stackedVal = stackOffset + p.val;
                  const nSeries = series.filter(
                    (s) => !s.stack || s.stack === ser.stack
                  ).length; // not used for stacked; keep simple
                  const groupW = plotW / categories.length;
                  const barW = isBar
                    ? 18
                    : Math.max(
                        12,
                        groupW / (ser.stack ? 1 : series.length) - 4
                      );
                  const x = isBar
                    ? pad.l +
                      (stackOffset / (scale.max - scale.min || 1)) * plotW
                    : xFor(ci) -
                      barW / 2 +
                      (ser.stack ? 0 : (sIdx % nSeries) * barW);
                  const y = isBar
                    ? pad.t + (ci * plotH) / categories.length + 4
                    : yFor(stackedVal);
                  const w = isBar
