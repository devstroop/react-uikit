...
function assertSize(name: string, value: number): void {
  if (!Number.isInteger(value) || value < 1 || value > 12) {
    throw new RangeError(`Column property ${name} value should be between 1 and 12.`);
  }
}

function assertOffset(name: string, value: number): void {
  if (!Number.isInteger(value) || value < 0 || value > 12) {
    throw new RangeError(`Column property ${name} value should be between 0 and 12.`);
  }
}

function assertOrder(name: string, value: number): void {
  if (!Number.isInteger(value) || value < 0 || value > 12) {
    throw new RangeError(`Column property ${name} value should be between 0 and 12 or first/last.`);
  }
}
...
export function Column({ className, style, ...props }: ColumnProps) {
  ...
  const {
    size: _s, offset: _o, sizeXs: _xs, offsetXs: _oxs, sizeSm: _sm, offsetSm: _osm,
    sizeMd: _md, offsetMd: _omd, sizeLg: _lg, offsetLg: _olg, sizeXl: _xl, offsetXl: _oxl,
    sizeXx: _xx, offsetXx: _oxx,
    order: _ord, orderXs: _oxs2, orderSm: _osm2, orderMd: _omd2, orderLg: _olg2, orderXl: _oxl2,
    orderXx: _oxx2,
    ...rest
  } = props;
