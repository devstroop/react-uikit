...
const TOKEN_FORMATTERS: Record<
  string,
  (p: DateParts, date: Date, locale: string) => string
> = {
...
export const Datepicker = forwardRef<HTMLInputElement, DatepickerProps>(
  function Datepicker(
    ...
    const [text, setText] = useState(() =>
      defaultValue != null
        ? formatDate(
            parseDateInput(defaultValue, format) ?? todayParts(),
            format,
            locale
          )
        : ''
    ...
    const displayedText =
      open && pending
        ...
        : isControlled
          ? value
            ? formatDate(
                parseDateInput(value, format) ?? todayParts(),
                format,
                locale
              )
            : ''
        ...
        >
          ...
          {Array.from({ length: 6 }, (_, rowIndex) => (
            <div
              role="row"
              key={rowIndex}
              className={styles['dx-datepicker-row']}
            >
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      cellDate === todayString
                        ? styles['dx-datepicker-day--today']
                        : null,
                      cellDate === selectedDate
                        ? styles['dx-datepicker-day--selected']
                        : null,
              ...
              <label key={field} className={styles['dx-datepicker-time-field']}>
                ...
                <div className={styles['dx-datepicker-time-control']}>
                  <input
                    ...
                    value={pad2(
                      (pending ?? currentParts ?? todayParts())[field]
                    )}
                    onChange={(event) =>
                      changeTimeField(field, event.target.value)
                    }
                    onKeyDown={(event) => {
          ...
          <>
            ...
            {showButton && (
              <button
                ref={triggerRef}
                type="button"
                className={[styles['dx-datepicker-trigger']]
                  .filter(Boolean)
                  .join(' ')}
