...
describe('Datepicker', () => {
  ...
  it('navigates days with arrow keys and moves roving focus', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole('gridcell', { name: /March 11, 2024/ })
      ).toHaveFocus()
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByRole('gridcell', { name: /March 18, 2024/ })
      ).toHaveFocus()
    );
  });
  ...
  it('respects min/max and marks disabled dates', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Open calendar' }));
    expect(
      screen.getByRole('gridcell', { name: /March 2, 2024/ })
    ).toHaveAttribute('aria-disabled', 'true');
    expect(
      screen.getByRole('gridcell', { name: /March 25, 2024/ })
    ).toHaveAttribute('aria-disabled', 'true');
    expect(
      screen.getByRole('gridcell', { name: /March 12, 2024/ })
    ).toHaveAttribute('aria-disabled', 'true');
    expect(
      screen.getByRole('gridcell', { name: /March 10, 2024/ })
