...
describe('Tree', () => {
  ...
  it('checks a parent and cascades to descendants', async () => {
    ...
    expect(
      screen.getByRole('checkbox', { name: 'Select Child 1-1' })
    ).toBeChecked();
  });
  ...
  it('excludes disabled descendants from cascade derivation and payloads', async () => {
    ...
    await user.click(
      screen.getByRole('checkbox', { name: 'Select Child 1-3' })
    );
    ...
    expect(
      screen.getByRole('checkbox', { name: 'Select Child 1-2' })
    ).not.toBeChecked();
  });
  ...
  it('respects controlled checkedKeys', async () => {
    ...
    expect(
      screen.getByRole('checkbox', { name: 'Select Child 2-1' })
    ).not.toBeChecked();
  });
  ...
  it('sets aria-multiselectable for multiple selectionMode', () => {
    const { unmount } = render(
      <Tree data={sampleData} selectionMode="multiple" />
    );
  ...
  it('expand/collapse via caret button with aria-label and aria-expanded', async () => {
    ...
    render(
      <Tree data={sampleData} onExpand={onExpand} onCollapse={onCollapse} />
    );
    ...
    expect(onExpand).toHaveBeenCalledWith(
      expect.objectContaining({ item: expect.objectContaining({ id: '2' }) })
    );
    expect(screen.getByText('Child 2-1')).toBeInTheDocument();
    // after expand, caret label changes
    expect(
      screen.getByRole('button', { name: 'Collapse Root 2' })
  ...
  it('selection single: clicking selects and fires onChange', async () => {
    ...
    render(
      <Tree data={sampleData} onChange={onChange} selectionMode="single" />
    );
    const child11 = screen.getByText('Child 1-1');
    await user.click(child11);
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({
        item: expect.objectContaining({ id: '1-1' }),
        selectedItem: expect.objectContaining({ id: '1-1' }),
      })
    );
  ...
  it('selection multiple: toggles selection and collects selectedItems', async () => {
    ...
    const onChange = vi.fn();
    render(
      <Tree data={sampleData} onChange={onChange} selectionMode="multiple" />
    );
    ...
    // toggle off
    await user.click(screen.getByText('Child 1-1'));
    const args = onChange.mock.calls.at(-1)?.[0] as {
  ...
  it('controlled selectedItem shows as selected', () => {
    ...
    render(
      <Tree data={sampleData} selectedItem={selected} selectionMode="single" />
    );
    const root1 = screen
      .getByText('Root 1')
      .closest('[role="treeitem"]') as HTMLElement;
  ...
  it('controlled selectedItems shows multiple selected', () => {
    render(
      <Tree
        data={sampleData}
        selectedItems={[sampleData[0]!, sampleData[2]!]}
    ...
    expect(
      screen.getByText('Root 3').closest('[role="treeitem"]')
    ).toHaveAttribute('aria-selected', 'true');
  });
  ...
  it('children getter overrides item.children', () => {
    const dataWithGetter: TreeItem[] = [
      {
        id: 'root',
        text: 'Root',
        children: [{ id: 'ignored', text: 'Ignored' }],
      },
    ];
    const children = (item: TreeItem) =>
      item.id === 'root'
        ? [{ id: 'real', text: 'Real Child' } as TreeItem]
        : undefined;
  ...
  it('template prop renders custom content', () => {
    render(
      <Tree
        data={sampleData}
        template={(item) => <span>TMPL-{item.text}</span>}
      />
    );
  ...
  it('lazy loadChildData expands async with loading indicator and aria-busy', async () => {
    const loadChildData = vi.fn(async (item: TreeItem) => {
      if (item.id === 'lazy')
        return [{ id: 'lazy-1', text: 'Lazy Child' } as TreeItem];
      return [];
    });
  ...
  it('supports LoadChildData alias', async () => {
    const LoadChildData = vi.fn(async () => [
      { id: 'c1', text: 'Child' } as TreeItem,
    ]);
    const lazyData: TreeItem[] = [{ id: 'p', text: 'Parent' }];
    render(<Tree data={lazyData} LoadChildData={LoadChildData} />);
    await userEvent
      .setup()
      .click(screen.getByRole('button', { name: 'Expand Parent' }));
  ...
  it('keyboard ArrowRight expands and moves to child, ArrowLeft collapses or moves to parent', async () => {
    ...
    fireEvent.keyDown(tree, { key: 'ArrowLeft' });
    await waitFor(() =>
      expect(root2Item).toHaveAttribute('aria-expanded', 'false')
    );
  });
  ...
  it('applies custom className', () => {
    const { container } = render(
      <Tree data={sampleData} className="my-tree" />
    );
