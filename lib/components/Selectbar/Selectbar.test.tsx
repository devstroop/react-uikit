...
describe("Selectbar", () => {
  ...
  it("toggles several options when multiple is set", async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    render(<Selectbar options={options} multiple onChange={onChange} aria-label="Range" />);
    expect(screen.getByRole("button", { name: "Day" })).toHaveAttribute("aria-pressed", "false");
  ...
  it("reflects a controlled array value when multiple is set", () => {
    const { rerender } = render(
      <Selectbar options={options} multiple value={["week"]} aria-label="Range" />,
    );
    expect(screen.getByRole("button", { name: "Week" })).toHaveAttribute("aria-pressed", "true");
    rerender(<Selectbar options={options} multiple value={[]} aria-label="Range" />);
    expect(screen.getByRole("button", { name: "Week" })).toHaveAttribute("aria-pressed", "false");
  });
  ...
  it("accepts un-narrowed union usage through the loose overload", () => {
    ...
    render(<Selectbar options={options} value={value} onChange={onChange} aria-label="Range" />);
    expect(screen.getByRole("button", { name: "Day" })).toHaveAttribute("aria-pressed", "true");
  });
