...
describe('QRCode', () => {
  ...
  it('supports custom label and size', () => {
    const { container } = render(
      <QRCode value="x" size={64} ariaLabel="Custom" />
    );
    const svg = container.querySelector('svg');
  ...
  it('draws the top-left finder outer corner', () => {
    const { container } = render(
      <QRCode value="hello" size={290} margin={4} />
    );
  ...
  it('grows the symbol for longer payloads', () => {
    const short = render(<QRCode value="hi" />).container.querySelectorAll(
      'svg rect'
    ).length;
    const long = render(
      <QRCode value={'0123456789'.repeat(30)} />
  ...
  it('renders an accessible placeholder and reports once when the payload exceeds capacity', () => {
    const onError = vi.fn();
    const { container } = render(
      <QRCode value={'x'.repeat(4000)} onError={onError} />
    );
  ...
  it('clamps negative margins to zero', () => {
    const { container } = render(
      <QRCode value="hello" size={100} margin={-3} />
    );
