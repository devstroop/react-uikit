...
describe('PickList', () => {
  ...
  it('defaults ariaLabel to PickList on root', () => {
    ...
    expect((container.firstChild as Element).getAttribute('aria-label')).toBe(
      'PickList'
    );
  });
  ...
  it('custom ariaLabel', () => {
    const { container } = render(
      <PickList source={source} target={target} ariaLabel="My Pick" />
    );
    expect((container.firstChild as Element).getAttribute('aria-label')).toBe(
      'My Pick'
    );
  ...
  it('supports AriaLabel alias', () => {
    const { container } = render(
      <PickList source={source} target={target} AriaLabel="Alias Pick" />
    );
    expect((container.firstChild as Element).getAttribute('aria-label')).toBe(
      'Alias Pick'
    );
  ...
  it('move selected to target moves items and fires callbacks', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Move selected to target' })
    );
  ...
  it('move selected to source moves back', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Move selected to source' })
    );
    expect(onMove).toHaveBeenCalledWith(
      expect.objectContaining({ direction: 'toSource' })
    );
    expect(
      screen.getByRole('listbox', { name: 'Source' }).textContent
    ).toContain('Egg');
  });
  ...
  it('move all to target moves non-disabled and fires allToTarget', async () => {
    ...
    render(<PickList source={source} target={target} onMove={onMove} />);
    await user.click(
      screen.getByRole('button', { name: 'Move all to target' })
    );
    expect(onMove).toHaveBeenCalledWith(
      expect.objectContaining({ direction: 'allToTarget' })
    );
    ...
    expect(
      screen.getByRole('listbox', { name: 'Target' }).textContent
    ).toContain('Apple');
  });
  ...
  it('move all to source moves non-disabled target items', async () => {
    ...
    render(<PickList source={source} target={target} onMove={onMove} />);
    await user.click(
      screen.getByRole('button', { name: 'Move all to source' })
    );
    expect(onMove).toHaveBeenCalledWith(
      expect.objectContaining({ direction: 'allToSource' })
    );
    expect(
      screen.getByRole('listbox', { name: 'Source' }).textContent
    ).toContain('Egg');
  ...
  it('supports keyProperty custom identity', async () => {
    ...
    const src = [
      { myId: 'a', text: 'A' },
      { myId: 'b', text: 'B' },
    ] as unknown as PickListItem[];
    const tgt: PickListItem[] = [];
    const onMove = vi.fn();
    render(
      <PickList source={src} target={tgt} keyProperty="myId" onMove={onMove} />
    );
    ...
    expect(onMove).toHaveBeenCalledWith(
      expect.objectContaining({
        moved: expect.arrayContaining([expect.objectContaining({ myId: 'a' })]),
      })
    );
  });
  ...
  it('keyboard ArrowDown/Up moves active and Enter/Space toggles selection', async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByRole('option', { name: 'Banana' })); // ensure we can still toggle via click after keyboard nav
  ...
  it('disabled items not moved on move selected', async () => {
    ...
    expect(screen.getByRole('option', { name: 'Carrot' })).toHaveAttribute(
      'aria-selected',
      'false'
    );
    ...
    expect(
      onMove.mock.calls[0]?.[0].moved.some((it: PickListItem) => it.id === '3')
    ).toBe(false);
  });

  it('applies custom className', () => {
    const { container } = render(
      <PickList source={source} target={target} className="my-pick" />
    );
