...
describe('ProfileMenu', () => {
  ...
  it('ArrowDown/Up moves active index', async () => {
    ...
    await user.keyboard('{ArrowDown}');
    expect(
      screen.getByRole('menuitem', { name: /Settings/ }).className
    ).toMatch(/active/);
  ...
  it('Home/End moves to first/last', async () => {
    ...
    expect(screen.getByRole('menuitem', { name: /Profile/ }).className).toMatch(
      /active/
    );
  });
  ...
  it('has nav landmark with ariaLabel', () => {
    ...
    expect(
      screen.getByRole('navigation', { name: 'User menu' })
    ).toBeInTheDocument();
  });
