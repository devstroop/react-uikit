...
const panes = [
  ...
  {
    label: 'Right',
    children: <div>Right content</div>,
    size: '60%',
    min: '10%',
    max: '90%',
  },
...
describe('Splitter', () => {
  ...
  it('renders separator handle with correct aria attributes', () => {
    ...
    expect(sep).toHaveAttribute(
      'aria-label',
      expect.stringContaining('Resize handle')
    );
  });
  ...
  it('keyboard vertical ArrowUp/Down resize', () => {
    const onResize = vi.fn();
    render(
      <Splitter panes={panes} orientation="vertical" onResize={onResize} />
    );
    const sep = screen.getByRole('separator');
  ...
  it('keyboard Home/End jump to min/max', () => {
    ...
    expect(onResize).toHaveBeenLastCalledWith(
      expect.objectContaining({ paneIndex: 0 })
    );
  ...
  it('renders collapse button when collapsible', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Collapse pane 1/ })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Collapse pane 1/ })
  ...
  it('toggles collapse on button click and fires onCollapse', async () => {
    ...
    expect(onCollapse).toHaveBeenCalledWith(
      expect.objectContaining({ paneIndex: 0, collapse: true })
    );
    // after collapse, expand button should appear
    expect(
      await screen.findByRole('button', { name: /Expand pane 1/ })
    ).toBeInTheDocument();
    const expandBtn = screen.getByRole('button', { name: /Expand pane 1/ });
    await user.click(expandBtn);
    expect(onCollapse).toHaveBeenLastCalledWith(
      expect.objectContaining({ paneIndex: 0, collapse: false })
    );
  });
  ...
  it('cancel prevents collapse', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Expand pane 1/ })
  ...
  it('initial collapsed pane is hidden', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Expand pane 1/ })
    ).toBeInTheDocument();
  });

  it('applies custom className', () => {
    const { container } = render(
      <Splitter panes={panes} className="my-split" />
    );
