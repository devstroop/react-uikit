...
export function Toc({
  ...
  const [activeSelector, setActiveSelector] = useState<string | null>(
  ...
  useEffect(() => {
      ...
      for (const item of items) {
        ...
        if (top <= 80) {
          if (
            !closestAbove ||
            top >
              closestAbove.el.getBoundingClientRect().top -
                (container !== window
        ...
        if (visible[0]) {
          ...
          for (const item of items) {
            ...
            if (
              ...
              target.id === item.selector.slice(1)
            ) {
              setActiveSelector(item.selector);
              break;
            }
    ...
    if (container === window) {
      ...
      (container as Element).addEventListener('scroll', scrollHandler, {
        passive: true,
      });
      updateActiveByScroll();
      return () => {
  ...
  return (
    <nav
      ...
      className={[styles.root, styles[orient], className]
        .filter(Boolean)
        .join(' ')}
    >
        ...
        {items.map((item) => {
          ...
          return (
            <li key={`${item.text}-${item.selector}`} className={styles.item}>
              <a
                href={
                  item.selector.startsWith('#') || item.selector.startsWith('.')
                    ? item.selector
                    : `#${item.selector}`
                }
