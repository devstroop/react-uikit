...
describe('Select', () => {
  ...
  it('gives options precedence over children', () => {
    render(
      <Select
        aria-label="Region"
        options={[{ value: 'a', label: 'From Options' }]}
      >
        <option value="b">From Children</option>
      </Select>
    );
    expect(
      screen.getByRole('option', { name: 'From Options' })
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('option', { name: 'From Children' })
    ).not.toBeInTheDocument();
  });

  it('sets aria-invalid only when invalid', () => {
    const { rerender } = render(
      <Select aria-label="Region" options={options} />
    );
  ...
  it('applies size and invalid classes', () => {
    ...
    rerender(
      <Select aria-label="Region" size="sm" invalid options={options} />
    );
  ...
  it('reports the selected value through onChange', async () => {
    ...
    render(
      <Select aria-label="Region" onChange={onChange} options={options} />
    );
